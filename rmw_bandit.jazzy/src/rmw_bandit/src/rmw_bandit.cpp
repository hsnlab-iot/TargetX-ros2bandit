...
const char *action_suffix = "_action/send_goal";
...
void zmq_send_publishers() {
    {
        ...
        for (auto& [topic, sent] : published_topics) {
            ...
            RCLCPP_DEBUG(logger, "Sening publisher info for topic: %s", topic.c_str());
...
void zmq_send_actions() {
    {
        ...
        for (auto& [action, sent] : action_servers) {
            if (sent) continue;  // Skip if already sent
            RCLCPP_DEBUG(logger, "Sening action_servers info: %s", action.c_str());
        ...
        for (auto& [action, sent] : action_clients) {
            if (sent) continue;  // Skip if already sent
            RCLCPP_DEBUG(logger, "Sening action_clients info: %s", action.c_str());
...
void zmq_send_nodename() {
    {
        ...
        for (auto &[nodename, sent] : nodenames) {
            ...
            RCLCPP_DEBUG(logger, "Sening nodename: %s", nodename.c_str());
...
void zmq_send_trigger(const std::string& topic) {
    ...
    RCLCPP_DEBUG(logger, "Sening trigger for topic: %s", topic.c_str());
...
void zmq_send_action_trigger(const std::string& action) {
    ...
    RCLCPP_DEBUG(logger, "Sening trigger for action: %s", action.c_str());
                        ...
                        if (type == "block") {
                            ...
                            if (!topic.empty()) {
                                ...
                                if (actions.find(action) != actions.end()) {
                                    ...
                                    if (elapsed_time < 333) {
                                        zmq_send_action_trigger(action);
                                    }
                                }
...
void load_rmw_library() {
    if (!rmw_lib_handle) {
        ...
        LOAD_REAL_SYMBOL(rmw_feature_supported);
        ...
        LOAD_REAL_SYMBOL(rmw_take_dynamic_message);
        LOAD_REAL_SYMBOL(rmw_take_dynamic_message_with_info);
        LOAD_REAL_SYMBOL(rmw_serialization_support_init);
...
extern "C" {
    ...
    rmw_service_t *rmw_create_service(
        ...
        const rmw_qos_profile_t *qos_policies) {
        load_rmw_library();
        RCLCPP_INFO(logger, "Creating service: %s", service_name);

        size_t name_len = std::strlen(service_name);
        size_t suffix_len = std::strlen(action_suffix);

        if ((name_len > suffix_len) && (std::strcmp(service_name + (name_len - suffix_len), action_suffix) == 0)) {
            std::string action_name(service_name);
            action_name.resize(name_len - suffix_len - 1);
            {
                std::lock_guard<std::mutex> lock(queue_mutex);
    ...
    bool rmw_feature_supported(rmw_feature_t feature) {
        load_rmw_library();
        return real_rmw_feature_supported(feature);
    }
    ...
    rmw_client_t *rmw_create_client(
        ...
        const char *service_name,
        const rmw_qos_profile_t *qos_policies) {
        RCLCPP_INFO(logger, "Create client %s", service_name);
        load_rmw_library();
        ...
        if ((name_len > suffix_len) && (std::strcmp(service_name + (name_len - suffix_len), action_suffix) == 0)) {
            ...
            {
                std::lock_guard<std::mutex> lock(queue_mutex);
                action_clients.push_back(std::pair<std::string, bool>(action_name, false));
            }
            RCLCPP_INFO(logger, "Action client created: %s", action_name.c_str());
            zmq_send_actions();
        }
    ...
    rmw_ret_t rmw_destroy_client(
        rmw_node_t *node,
        rmw_client_t *client) {
        RCLCPP_INFO(logger, "Destroying client %s", client->service_name);
        load_rmw_library();
        ...
        if ((name_len > suffix_len) && (std::strcmp(service_name + (name_len - suffix_len), action_suffix) == 0)) {
            ...
            {
                std::lock_guard<std::mutex> lock(queue_mutex);
                action_clients.erase(std::remove_if(action_clients.begin(), action_clients.end(),
    ...
    rmw_ret_t rmw_send_request(
        ...
        int64_t *sequence_id) {
        RCLCPP_INFO(logger, "Request on %s", client->service_name);
        load_rmw_library();
        ...
        if ((name_len > suffix_len) && (std::strcmp(service_name + (name_len - suffix_len), action_suffix) == 0)) {
            std::string action_name(service_name);
            action_name.resize(name_len - suffix_len - 1);
            RCLCPP_DEBUG(logger, "Action client request: %s", action_name.c_str());
    ...
    rmw_ret_t rmw_take_dynamic_message(
        ...
        rmw_subscription_allocation_t *allocation) {
        load_rmw_library();
        return real_rmw_take_dynamic_message(subscription, dynamic_message, taken, allocation);
    }
    ...
    rmw_ret_t rmw_take_dynamic_message_with_info(
        ...
        rmw_subscription_allocation_t *allocation) {
        load_rmw_library();
        return real_rmw_take_dynamic_message_with_info(subscription, dynamic_message, taken, message_info, allocation);
    }
    ...
    rmw_ret_t rmw_serialization_support_init(
        ...
        load_rmw_library();
        return real_rmw_serialization_support_init(serialization_lib_name, allocator, serialization_support);
    }
}
